...
class ThirdDigit extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      value: 0,
  ...
  updateDigitValue = ({ currentTarget }) => {
    ...
    updateValues('s1', this.state.value + 1);
        break;
      case 'decrease' :
        this.setState ((previousState) => ({
          value: previousState.value - 1,
  ...
  buttonIncrease = () => {
    if (this.state.isHovered) {
      return (
        <button 
        className={this.state.value < 5 ? "digit-value-change-button increase-button" : "digit-value-change-button disabled" }
        name="add" 
        onClick={ this.state.value < 5 ?  this.updateDigitValue : () => {} } >
